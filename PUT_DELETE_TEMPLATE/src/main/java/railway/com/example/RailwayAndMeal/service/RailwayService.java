...
public class RailwayService {
	...
	/**
	 * Add code for throwing custom exceptions in getTicketByPnr(),
	 * deleteTicketByPnr(), updateTicket() and addTicket() method snippets.
	 **/
	...
	public void deleteTicketByPnr(long pnr) {
		if (ObjectUtils.isEmpty(ticketMap.get(pnr))) {
			throw new TicketNotFoundException("There is no ticket with pnr: " + pnr);
		} else {
	...
	public void updateTicket(Ticket ticket) {
		if (ObjectUtils.isEmpty(ticketMap.get(ticket.getPnr()))) {
			throw new TicketNotFoundException("There is no ticket with pnr: " + ticket.getPnr());
		} else {
